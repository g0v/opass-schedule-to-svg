...
- **批量匯出**: 支援一鍵打包下載所有日期、所有廳別的議程圖檔以及樣式表 (ZIP)。
...
2. **載入議程**：支援輸入 OPass 格式的 JSON URL 或直接上傳 `schedule.json` 檔案。
3. **載入樣式**：同樣支援 URL 或檔案上傳自訂的 `style.config.json` 樣式檔。
4. **批量匯出**：設定完成後，點擊「Download All」，程式會自動生成所有組合並與樣式檔一起打包成一個 ZIP 檔後下載。
